...
export default function NovuNotifications({ variant = 'header-desktop' }: NovuNotificationsProps) {
  ...
  // Check if Novu is properly configured (not undefined and not placeholder)
  const isConfigured = 
    !!appIdentifier && 
  ...
  if (variant === 'header-desktop') {
    ...
    buttonClass = "relative p-2 text-gray-400 hover:text-white transition-colors rounded-xl hover:bg-white/10 cursor-pointer flex items-center justify-center";
    iconClass = "w-5 h-5 text-gray-400 hover:text-white transition-colors";
  }
